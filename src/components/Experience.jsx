...
const CardBody = forwardRef(({ item, inView, slideClass, isWork }, ref) => (
  <div
    ref={ref}
    className={`transition-all duration-500 ${inView ? slideClass : 'opacity-0 translate-y-4'}`}
  >
    <div className="bg-gray-800/60 border border-gray-700/50 hover:border-indigo-500/40 rounded-2xl p-5 shadow-lg hover:shadow-indigo-900/20 transition-all duration-300 animate-tilt-card">
      ...
      {isWork && (
        <div className="flex flex-wrap gap-1.5 animate-stagger">
          {item.tech.map((t) => (
            <span key={t} className="px-2 py-0.5 text-[11px] font-medium bg-indigo-600/15 border border-indigo-500/25 text-indigo-300 rounded-md">
              {t}
            </span>
          ))}
        </div>
      ...
      <div className="max-w-5xl mx-auto">
        <div ref={headingRef} className="text-center mb-16">
          ...
          <h2 className="inline-block text-4xl font-bold text-white relative">
            ...
            <span className={`absolute -bottom-2 left-0 right-0 h-0.5 rounded-full bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-500 animate-shimmer transition-all duration-700 origin-left ${headingInView ? 'scale-x-100 opacity-100' : 'scale-x-0 opacity-0'}`} />
          </h2>
        </div>

        <div className="relative" ref={lineRef}>
          <div className={`md:hidden absolute left-[11px] top-0 bottom-0 w-px bg-gradient-to-b from-indigo-500/60 via-purple-500/40 to-indigo-500/20 origin-top ${lineInView ? 'animate-draw-line' : 'scale-y-0'}`} />
          <div className={`hidden md:block absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-indigo-500/60 via-purple-500/40 to-indigo-500/20 origin-top ${lineInView ? 'animate-draw-line' : 'scale-y-0'}`} />
          <div className="flex flex-col gap-10">
            {items.map(({ data, side, dot }, i) => (
              <TimelineItem key={i} item={data} side={side} dotColor={dot} />
            ))}
          </div>
        </div>
      </div>
